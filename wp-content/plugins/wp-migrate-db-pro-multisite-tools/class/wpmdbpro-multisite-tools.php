...
class WPMDBPro_Multisite_Tools extends WPMDBPro_Addon {
	...
	function __construct( $plugin_file_path ) {
		...
		if ( ! $this->meets_version_requirements( '1.5.1' ) ) {
			return;
		}
		...
		if ( $this->wpmdbpro->table_is( 'usermeta', $table_name ) ) {
			if ( 1 == $blog_id ) {
				...
				if ( 0 === stripos( $row->meta_key, $prefix ) ) {
					// Rename prefixed keys.
					$row->meta_key = substr_replace( $row->meta_key, $new_prefix, 0, strlen( $prefix ) );
				} elseif ( 0 === stripos( $row->meta_key, $wpdb->prefix ) ) {
	 ...
	 * @return bool|string
	 */
	public function get_subsite_id( $subsite ) {
		foreach ( wp_get_sites( array( 'limit' => 0 ) ) as $site ) {
			...
			if ( is_numeric( $subsite ) ) {
				...
				$subsite      = untrailingslashit( $this->scheme_less_url( $subsite ) );
				$subsite_path = untrailingslashit( $this->scheme_less_url( get_blogaddress_by_id( $blog_id ) ) );

				if ( $subsite == $subsite_path ) {
					return $blog_id;
				}
			}
		}
