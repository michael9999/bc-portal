(function( $ ) {
	...
	function table_is( table_prefix, desired_table, given_table ) {
		if ( ( table_prefix + desired_table ).toLowerCase() === given_table.toLowerCase() ) {
			return true;
		}

		var escaped_given_table = preg_quote( given_table );
	...
	function is_subsite_table( table_prefix, table_name ) {
		...
		if ( undefined !== selected_subsite.blog_id ) {
			if ( 1 < selected_subsite.blog_id ) {
				...
				var escaped_table_name = preg_quote( table_name );
	...
	function filter_table_for_subsite( exclude, table_name ) {
		...
		if ( is_multisite_subsite_export() ) {
			...
			if ( table_is( table_prefix, 'users', table_name ) || table_is( table_prefix, 'usermeta', table_name ) ) {
				return exclude;
			}
			...
			$.each( ms_tables, function( index, ms_table ) {
				if ( table_is( table_prefix, ms_table, table_name ) ) {
					exclude = true;
				}
			} );
	...
	function validate_new_prefix( new_prefix ) {
		var escaped_new_prefix = preg_quote( new_prefix );
